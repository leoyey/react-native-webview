...
class Web3JsInjectorClient {
    ...
    private String rpcUrl = "https://mainnet.infura.io";

    Web3JsInjectorClient(Context context) {
    ...
    Web3JsInjectorResponse loadUrl(final String url, String userAgent) {
    ...
    @Nullable
    Web3JsInjectorResponse loadUrl(final String url, final Map<String, String> headers) {
        Request request = buildRequest(url, headers);
        Web3JsInjectorResponse result = null;
        try {
    ...
    String assembleJs(Context context, String template) {
        if (TextUtils.isEmpty(jsLibrary)) {
            jsLibrary = loadFile(context, R.raw.tko_min);
        }
    ...
    @Nullable
    private Web3JsInjectorResponse buildResponse(Response response) {
        String body = null;
        ...
        return new Web3JsInjectorResponse(result, code, finalUrl, mime, charset, isRedirect);
    }
...
//        }
    ...
    private String loadInitJs(Context context) {
        String initSrc = loadFile(context, R.raw.tko_init);
        String address = walletAddress == null ? "0x0000000000000000000000000000000000000000" : walletAddress;
    ...
    private OkHttpClient createHttpClient() {
        return new OkHttpClient.Builder()
                .cookieJar(new Web3WebViewCookieJar())
                .build();
    }
}
